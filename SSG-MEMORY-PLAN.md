...
**Status:** Phase 0 (stop-loss) ✅ complete · Phase 1 mitigations ✅ shipped (`v1.0.0-beta.35`, published) · **Phase 2 durable bound ✅ implemented + verified (`cms-ssg-build`), pending `beta.36` publish** · Phase 3 (fleet rollout) pending.
...
- **Recommendation — two tracks.**
  ...
  - **Track 2 ✅ shipped (beta.36) — the durable bound:** a framework-owned, opt-in **`cms-ssg-build`** command that renders the routes as **N separate `vite-ssg build` processes** (each a slice of the routes, merged into `dist/`), so peak heap is `O(shard size)`, **independent of total route count**. Proven: `site-peepoo` (662 routes — normally needs an 8 GB heap) builds green with **each shard capped at 2 GB**. Shipped as the process-per-shard variant (Option C) rather than build-once (Option D) for fleet robustness — see §3. Retire the `LARGE_16GB` + `NODE_OPTIONS` stopgaps once sites adopt it.
...
### Track 2 — The durable bound: `cms-ssg-build` ✅ SHIPPED (beta.36; opt-in)
Implemented as **Option C (process-per-shard), not Option D (build-once)** — a deliberate reversal of the §2 preference once the two were weighed against *this* fleet's realities. Option D's build-once avoids re-bundling but requires either forking vite-ssg's minified `build()` or reimplementing its render loop; both are permanent maintenance liabilities that break whenever vite-ssg updates, across 26 non-dev-maintained sites. Option C needs **zero vite-ssg internals** — it just shards the framework's own `includedRoutes` and runs N ordinary `vite-ssg build` processes into separate outDirs, then merges. Perfect per-shard fidelity, robust across vite-ssg versions; the only cost is re-bundling per shard (build time), which is opt-in and only paid by sites large enough to need the bound.

What shipped:
- **`bin/cms-ssg-build.js` + `scripts/ssg-build/index.mjs`** — estimates the route count, picks `N = ceil(count / shardSize)`, runs shard `k/N` as its own `vite-ssg build` (env `CMS_SSG_SHARD` + `CMS_SSG_OUTDIR`), merges the shard dirs into `dist/`. **Sequential by default** (lowest peak memory + no race on the plugin's shared site-root writes); small sites fall through to a single ordinary build. Tunable via `--shard-size` / `--heap` / `--concurrency`.
- **Framework hooks** (env-gated, inert for a normal build): `includedRoutes` honors `CMS_SSG_SHARD` via the pure, unit-tested `applyRouteShard` (`/404` + `/admin` kept in every shard so each emits its own `404.html`); the **client** build honors `CMS_SSG_OUTDIR` (guarded off the SSR build via `env.isSsrBuild`, since vite-ssg imports the server bundle back from its own temp outDir).
- **Merge** unions the shard dirs (shared assets are byte-identical, route HTMLs disjoint); the home `index.html` is restored from whichever shard actually rendered `/` (others left a client shell).

**Verified** (§5): `site-erea` (60 routes) — merged `dist/` identical to a normal build after normalizing vite-ssg's own per-run preload non-determinism; `site-peepoo` (662 routes) — green with **2 GB per shard** vs. the 8 GB a single-process build needs.

**Not adopted:** upstream vite-ssg PR (E2) is still worth filing (a render-slice / window-close mode would let a future build-once variant drop the per-shard re-bundle), but Option C stands on its own with no external dependency.
...
**Phase 2 — Track 2 ✅ IMPLEMENTED (beta.36; opt-in `cms-ssg-build`).** Shipped the process-per-shard render (Option C) + the `applyRouteShard` / `CMS_SSG_OUTDIR` framework hooks (630 unit tests). Verified on `site-erea` (output identical to a normal build modulo vite-ssg's own preload non-determinism — §5) and `site-peepoo` (662 routes green at **2 GB/shard**, 5 shards, ~65 s). **Remaining roll:** publish `beta.36`; then dogfood by pointing a heavy site's `build:ssg` at `cms-ssg-build` (and dropping its `8192` flag) before flipping the template default in Phase 3.
...
1. **Synthetic scaling harness (deterministic OOM + curve).** Generate a synthetic site with N pages × L locales (drive N ∈ {100, 300, 600, 1200, 2400}). For each, run the build capped at a fixed heap (e.g. `--max-old-space-size=2048`) and record peak RSS/heap.
   ...
   - ✅ **Real-site proof already in hand:** `site-peepoo` (662 routes — a single-process build needs 8 GB and OOMs at 4 GB) built green with `cms-ssg-build --shard-size 150 --heap 2048`, i.e. **2 GB per shard**, all 662 routes rendered. The synthetic harness remains worth adding as a CI regression guard (a smoke build under a tight cap that a single-process build fails).
...
3. **Output fidelity ✅ (done for Option C).** ⚠️ Correction to the original plan: **`vite-ssg build` is not byte-reproducible run-to-run** — two identical normal builds already differ on a few pages' `modulepreload` chunk selection (equivalent chunks both present, nothing dropped; a `renderPreloadLinks` ordering artifact). So a raw `diff -r` is the *wrong* gate — it fails against the baseline itself. The gate actually used: `diff -r` normal vs. sharded `dist/` **after normalizing out `modulepreload` + `/assets/*` stylesheet links**, plus a per-page preload-count check (no drops). On `site-erea` the normalized trees are **identical**, and per-route HTML / `404.html` / `sitemap.xml` / `robots.txt` / canonical/hreflang/OG/JSON-LD / `dirStyle: nested` all match. (Filing the upstream determinism issue would let this gate return to a raw byte-diff.)
