// Pure CSS-variable derivation from a theme manifest. Lives in its own
// file (with no Vite-specific imports) so it can be called from both
// the runtime themeManager AND the build-time vite plugin, which uses
// it to emit `virtual:cms-theme-vars.css` — a synchronously-linked
// stylesheet containing every registered theme's tokens. That, combined
// with the SSR-rendered `data-site-theme` attribute (see main.js
// applySiteTheme), eliminates the FOUC dark-themed sites used to suffer.
...
export function buildCssVarMap(manifest) {
  if (!manifest) return {};
  const vars = {};
...
/**
 * Render the var map as a CSS rule body.
 * @param {object} vars Map of CSS custom property name → value
 */
export function renderCssVarRule(selector, vars) {
